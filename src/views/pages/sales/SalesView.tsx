...
import { useCartStore, useForm, useProductStore, useWarehouseStore } from "@/hooks";
import { FormControl, Grid, InputLabel, MenuItem, Select, Stack, Table, TableBody, Typography } from "@mui/material";
import { useEffect, useState } from "react";
import { CartView, ProductsStoreView } from ".";
...
export const SalesView = () => {
  const { name, search, onInputChange } = useForm(formFields);
  ...
  // const { products = [], cartProducts = [], warehouses = [] } = useSelector((state) => state.warehouses);
  ...
  return (
    <Grid container>
      <Grid item xs={12} sm={warehouse != null ? 5 : 12} sx={{ padding: '5px' }}>
        <Stack spacing={1}>
          <Typography variant="h5">Punto de venta {warehouse}</Typography>
        </Stack>
              ...
              <TableBody>
                {products
                  ...
                  .map((product: ProductModel) => {
                    const newProduct = {
                      ...product,
                      productStatus: [...product.productStatus.filter((productStatus: ProductStatusModel) => !cartProducts.filter((cart: CartModel) => cart.warehouseId.id === warehouse).map((e: any) => e.productStatus.id).includes(productStatus.id))]
                    };
      ...
      </Grid>
      {
        ...
        <Grid item xs={12} sm={7} sx={{ padding: '5px' }}>
          ...
          <Grid item xs={12} sm={12} sx={{ padding: '5px' }}>
            {/* <DetailsSale cartProducts={cartProducts} warehouseId={warehouse} /> */}
          </Grid>
        </Grid>
      }
    </Grid >
